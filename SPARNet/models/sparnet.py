...
class SPARNet(nn.Module):
    ...
    def __init__(
        ...
        n_ch = ch_clip(max_ch // int(np.log2(in_size // min_feat_size) + 1)) # ch_clip(43) = 43

        # ------------ define encoder --------------------
        self.encoder = []
        self.encoder.append(ConvLayer(3, n_ch, 3, 1))
        hg_depth = int(np.log2(64 / bottleneck_size)) # 4
        for i in range(down_steps): # 3
            cin, cout = ch_clip(n_ch), ch_clip(n_ch * 2) # 43, 86; 86, 128; 128, 128
            self.encoder.append(ResidualBlock(cin, cout, scale='down', hg_depth=hg_depth, att_name=att_name, **nrargs))
            n_ch = n_ch * 2 # 86; 172; 344
            hg_depth = hg_depth - 1 # 3; 2; 1
        hg_depth = hg_depth + 1 # 2
        self.encoder = nn.Sequential(*self.encoder)

        # ------------ define residual layers --------------------
        self.res_layers = []
        for i in range(res_depth + 3 - down_steps): # 10
            channels = ch_clip(n_ch) # 128
            self.res_layers.append(ResidualBlock(channels, channels, hg_depth=hg_depth, att_name=att_name, **nrargs))
        self.res_layers = nn.Sequential(*self.res_layers)

        # ------------ define decoder --------------------
        self.decoder = []
        for i in range(up_steps): # 3
            hg_depth = hg_depth + 1 # 3; 4; 5
            cin, cout = ch_clip(n_ch), ch_clip(n_ch // 2) # 128, 128; 128, 86; 86, 43
            self.decoder.append(ResidualBlock(cin, cout, scale='up', hg_depth=hg_depth, att_name=att_name, **nrargs))
            n_ch = n_ch // 2 # 172; 86; 43

        self.decoder = nn.Sequential(*self.decoder)
        self.out_conv = ConvLayer(ch_clip(n_ch), 3, 3, 1) # (43, 3, 3, 1)
